$oldBasicLBName = 'lb-basic01'
$foldBasicLBRG = 'rg-001-basicLBSingleFE'
$newStandardLBName = "lb-standard01"
...
# update vmss lb config
ForEach ($backendPool in $backendConfigs_Basic) {
    $vmssUsingBEP = Search-AzGraph -Query @"
    resources
        | where type == 'microsoft.compute/virtualmachinescalesets'
        | mv-expand nics = properties.virtualMachineProfile.networkProfile.networkInterfaceConfigurations
        | mv-expand ipconfigs = nics.properties.ipConfigurations
        | mv-expand beps = ipconfigs.properties.loadBalancerBackendAddressPools
        | where beps.id =~ '$($backendPool.Id)'
        | project vmssId = id, networkInterfaceConfigurationName = tostring(nics.name), ipConfigurationName = tostring(ipconfigs.name), resourceGroup, name
        | summarize ipConfigurationNames=make_list(ipConfigurationName) by vmssId,networkInterfaceConfigurationName,resourceGroup,name
"@

    # create new LoadBalancerBackendAddressPools of type System.Collections.Generic.IList`1[Microsoft.Azure.Management.Compute.Models.SubResource] with both BEP IDs
    $genericNewIpConfig = New-AzVmssIpConfig -LoadBalancerBackendAddressPoolsId $backendPool.Id,($backendPool.Id -replace $oldBasicLBName,$newStandardLBName) ` -Name 'noname' 

    Foreach ($vmss in $vmssUsingBEP) {
        $vmssObj = Get-AzVmss -ResourceGroupName $vmss.ResourceGroup -VMScaleSetName $vmss.name
        $vmssVMs = $vmssObj | Get-AzVmssVM

        ForEach ($ipConfigName in $vmss.ipConfigurationNames) {
            ...
            try {
                Update-AzVmss -ResourceGroupName $vmssObj.ResourceGroupName -VMScaleSetName $vmssObj.Name -VirtualMachineScaleSet $vmssObj -ErrorAction Stop
            }
            catch {
