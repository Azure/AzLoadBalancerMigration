...
function NsgCreationVmss {
    ...
    log -Message "[NsgCreationVmss] Initiating NSG Creation"
...
"@
        ...
        log -Message "[NsgCreationVmss] Adding one NSG Rule for each Load Balancing Rule"
        ...
        foreach ($loadBalancingRule in $loadBalancingRules) {
            ...
            log -Message "[NsgCreationVmss] Adding NSG Rule Named: $($networkSecurityRuleConfig.Name) to NSG Named: $($nsg.Name)"
            $nsg | Add-AzNetworkSecurityRuleConfig @networkSecurityRuleConfig > $null
            $priorityCount++
        }

        # Adding NSG Rule for each inboundNAT Rule
        log -Message "[NsgCreationVmss] Adding one NSG Rule for each inboundNatRule"
        ...
        # Saving NSG
        log -Message "[NsgCreationVmss] Saving NSG Named: $($nsg.Name)"
        try {
            $ErrorActionPreference = 'Stop'
            ...
            $message = @"
            [NsgCreationVmss] An error occured while adding security rules to NSG '$("NSG-"+$vmss.Name)'. TRAFFIC FROM LOAD BALANCER TO BACKEND POOL MEMBERS WILL
...
Export-ModuleMember -Function NsgCreationVmss
