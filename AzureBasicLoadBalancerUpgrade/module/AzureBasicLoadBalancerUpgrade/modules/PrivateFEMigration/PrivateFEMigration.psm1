...
function PrivateFEMigration {
    ...
    foreach ($feConfig in $basicLoadBalancerFeConfig) {
        ...
        If (!$ipAvailability.Available) {
            $message = "[PrivateFEMigration] The private IP address '$privateIP' in VNET '$vnetName', resource group '$vnetRG' is not available for allocation; another new device may have claimed it. To recover, remove the device that claimed the IP '$privateIP' from the VNET, then follow the steps at https://aka.ms/basiclbupgradefailure to retry the migration.`nError message: $_"
        ...
        try {
            ...
            $message = "[PrivateFEMigration] Failed to add FrontEnd Config '$($feConfig.Name)'. To recover address the following error, then follow the steps at https://aka.ms/basiclbupgradefailure to retry the migration.`nError message: $_"
            log 'Error' $message -terminateOnError
        }
    }
    ...
    try {
        ...
        $message = "[PrivateFEMigration] An error occured when moving private IPs to the new Standard Load Balancer. To recover address the following error, and then follow the steps at https://aka.ms/basiclbupgradefailure to retry the migration. `nError message: $_"
        log 'Error' $message -terminateOnError
    }
